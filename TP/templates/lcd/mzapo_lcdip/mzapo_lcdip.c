/*******************************************************************
  Simple program to show ip address on LCD by MicroZed
...
int main(int argc, char *argv[]) {
    ...
    for (ifa_it = ifa; ifa_it != NULL; ifa_it = ifa_it->ifa_next) {
        ...
        if (!(ifa_it->ifa_flags & 0x0000000001) ||  //needs to be up
            (ifa_it->ifa_flags & 0x0000000008) || // not loopback
            (ifa_it->ifa_addr->sa_family != AF_INET)) {
            continue;
        }

        ip_address = ntohl(((struct sockaddr_in *) (ifa_it->ifa_addr))->sin_addr.s_addr);
        break;
    }
    ...
    for (i = 0; i < 320; i++) {
        for (j = 0; j < 8; j++) {
            blank = 1;
            bl_y = i % 40;
            ip_byte_num = 3 - i / 40 + 2;
            if (ip_byte_num <= 3) {
                ip_bit = (ip_address >> (8 * ip_byte_num + 7 - j)) & 0x01;
                blank = 0;
                /* printf("ip_byte_num %d, ip_bit %d = %d\n", ip_byte_num, j, ip_bit); */
            } else {
                ip_bit = 0;
            }
